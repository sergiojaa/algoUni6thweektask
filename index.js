bbbbbbb//1.რიცხვების მასივის გათვალისწინებით, გამოიყენეთ ფილტრი ახალი მასივის შესაქმნელად, რომელიც შეიცავს მხოლოდ ლუწი რიცხვებს.
...
let arr = [];
let array2 = array.map((lastname) =>  lastname.lastname )
...
const words = 'i am sergi kaliashvili'
function isPangram(words){
    let word = 'i am sergi kaliashvili'
...
let str = 'he is sergi kaliashvili on the first course'
let arr = str.split(' ')
let set = [...new Set(arr)]
let sort = set.sort()
let filter = sort.filter((item)=> item !== 'the' && item !== 'a' && item !== 'is' )
console.log(filter)
